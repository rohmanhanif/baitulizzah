...
var FormTranslation = {
  ...
  setText: function(e){
    ...
    if (window.FORM_MODE === 'cardform' && regExp.test(e.innerHTML)) {
      type = 'innerHTML';
    }
  ...
  translateFormElements: function(formElementList){
    ...
    $A(formElementList).each(function(e){
      if (e && e.hasClassName && e.hasClassName('noTranslate')) {
        ...
        var jfUploadHeading = e.querySelector(".jfUpload-heading");
        if (jfUploadHeading) {
          self.setText(jfUploadHeading);
        }
  ...
  setDefaultDir: function () {
    ...
    var form = $$('.form-all')[0];
    if (form && self.to && self.rtlLanguages && self.rtlLanguages.indexOf(self.to) > -1) {
      form.setAttribute('dir', 'rtl');
    }
  },
